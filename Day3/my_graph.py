...
import svgutils.transform as sg  # pip install svgutils -- replaces "from graph_utils import svgutils as sg"
import os, string
import numpy as np
...
def build_bar_legend(X, n_tick, ax, mymap, label='$\\nu$ (Hz)',
    ...
    # Determine the scale and bounds
    if scale == 'linear':
        if bounds is None:
            bounds = [X[0], X[-1]]  # Simplified bounds for linear scale
        norm = mpl.colors.Normalize(vmin=bounds[0], vmax=bounds[1])
        bounds = np.linspace(bounds[0], bounds[1], color_discretization)  # Adjusted bounds for color discretization
    elif scale == 'log10':
        if bounds is None:
    ...
    cb = mpl.colorbar.ColorbarBase(ax, cmap=mymap, norm=norm,
                                    boundaries=bounds, orientation=orientation)

    # Set ticks explicitly
    if ticks_labels is None:
        ticks = np.linspace(X[0], X[-1], n_tick)  # Generate equi-spaced ticks
        cb.set_ticks(ticks)
        cb.set_ticklabels([f"{tick:.1f}" for tick in ticks])  # Format tick labels as floats
    else:
        cb.set_ticks(X)  # Set custom ticks if provided
...
def build_bar_legend_old(X, n_tick, ax, mymap, label='$\\nu$ (Hz)',
                          bounds=None, ticks_labels=None,
                          orientation='vertical', scale='linear',
                          color_discretization=None):
    """ X -> ticks """
    ...
    # cb.set_ticks(X)
    cb.set_ticks(np.linspace(X[0], X[-1], n_tick))
    # cb.set_ticks(np.round(np.arange(X[0], X[-1], X[1]-X[0])))
...
def put_list_of_figs_to_svg_fig(FIGS, CAP_SIZE=14,
                                 ...
                                 transparent=True):
    """ take a list of figures and make a multi-panel SVG figure, labeling
    each panel A, B, C, ... Requires the "svgutils" package (pip install
    svgutils), used here in place of the former graph_utils wrapper. """
